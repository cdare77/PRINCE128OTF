...
// State in form {0x0123456789abcdef} where f is highest bit
...
static void addRoundKey(uint8_t const * const Key);
 ...
 */
static void keySchedule(uint8_t const * const Key, uint8_t * const subkey)
{
...
void decipher(uint8_t * const state, uint8_t * const Key)
{
    ...
    for (Round = 0; Round < 8; Round++)
    {
        temp[Round] = subkey[Round];
        subkey[Round] = Key[Round];
        Key[Round] = temp[Round];
        state[Round] ^= Key[Round];
    } // End for-loop
